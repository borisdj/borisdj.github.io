...
So the following proposal is an attempt to find a fix. But first to address two major issues:
...
&nbsp; The second concern is that large corporations are profiting significantly from much of OS Software while giving back very little or none at all. They take dozens of OSS and encapsulate it into a commercial proprietary app or service while some of them make huge incomes. Which is fine as such but there should be a way to have them pay their fair share to the Engineers behind those projects.  
...
It enables [sustainability](https://thenewstack.io/this-week-in-programming-a-manifesto-for-sustainable-open-source-development/){:target="_blank"} and ensures the project survives in the long term. What's more gives good incentives and creates a self-reinforcing positive loop by encouraging users to give back via 'code commits'. Additionaly, they should have simple progressive pricing tiers (based on number of Devs or some other metric). And even the top tier should not be too expensive, to be easily acceptable cost for such companies and proportional to the value they get. For example, tiers could be in range from several hundreds to a few thousand per yearly subscription, or alternatively to also have an option for perpetual (permanent) license. Regular funding would also make it possible to have a reward model for impactful collaborators. 
Also when setting prices avoid please amounts like 999 (just for 'advertising'), instead use rounded numbers, plain 1000 is fine. Finally, you can consider accepting *Bitcoin* as payment, as it aligns with the principles *OpenSource Money* (more info in previous blog posts).
